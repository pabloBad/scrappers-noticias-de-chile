...
START_URLS = [
    ...
    'https://www.biobiochile.cl/lista/categorias/sociedad',
...
class BiobioSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        next_page = response.css(
            "li.page-item:nth-child(13) > a:nth-child(1)::attr(href)").get()
    ...
    def parse_index(self, response):
        ...
        for item in response.css("div.noticia.row"):
            ...
            biobio_item = BiobioItem()

            link_to_content = item.css(
                "div.col-xs-6.titular-fecha a::attr(href)").get()

            biobio_item["publication_date"] = item.css(
                "div.col-xs-2.fecha p::text").get()
            biobio_item["publication_hour"] = item.css(
                "div.col-xs-2.fecha span::text").get()
            biobio_item["author"] = item.css("p.autor a::text").get()
            biobio_item["author_link"] = item.css(
                "p.autor a::attr(href)").get()
            biobio_item["title"] = item.css(
                "div.col-xs-6.titular-fecha a h1::text").get()
            biobio_item["link"] = link_to_content

            content_request = scrapy.Request(link_to_content,
                                             callback=self.parse_content,
                                             dont_filter=True)
            content_request.meta["biobio_item"] = biobio_item

            yield content_request

    def parse_content(self, response):
        biobio_item = response.meta['biobio_item']

        # Content Processing

        # Get all data in the content
        content_list = response.css(
            "div.nota-contenido.text-19.robotos *::text").getall()
        ...
        tags = [
            text.strip()
            for text in response.css("div.nota-etiquetas *::text").getall()
            if text.strip() != ""
        ]
        ...
        biobio_item["embedded_links"] = [
            link for link in content_list if url(link)
        ]
    ...
    def delete_gallery(self, content_list):
        contains_gallery = [
            True if '\n\t#lightgallery' in text else False
            for text in content_list
        ]
        try:
    ...
    def delete_tags(self, content_list):
        contains_tags = [
            True if 'Etiquetas de esta nota:' in text else False
            for text in content_list
        ]
        try:
    ...
    def delete_banners(self, content_list):
        contains_banners = [
            True if
            '\n\t\t\t\t\t\t\t\t{{#en_desarrollo}}\n\t\t\t\t\t\t\t\t\t' in text
            else False for text in content_list
        ]
        try:
